#include <bits/stdc++.h>
using namespace std;
long long helper(long long idx, long long val, vector<long long> &R, vector<long long> &C, long long N, long long F, vector<long long> &dp)
{
    if (idx == N - 1)
    {
        return dp[idx] = 0;
    ...
    for (int j = idx + 1; j < N; j++)
    {
        ...
        long long next = helper(j, val, R, C, N, F, dp);
        if (next == LLONG_MAX / 2)
            continue;
...
bool feasible(long long X, vector<long long> &R, vector<long long> &C, long long N, long long F)
{
    ...
    long long cost = helper(0, X, R, C, N, F, dp);
